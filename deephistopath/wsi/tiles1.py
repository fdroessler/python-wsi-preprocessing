...
SUMMARY_TITLE_FONT_PATH = "/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf"
...
def tile_to_pil_tile(tile, mode=0):
  ...
  # tile_region = s.read_region((x, y), 4, (int(np.ceil(w)), int(np.ceil(h))))
  # RGBA to RGB
...
def tile_to_np_tile(tile, mode=0):
  ...
  np_img = np.ndarray(
      ...
      dtype=np.uint8,
      shape=(pil_img.height, pil_img.width, pil_img.bands)
  )
  return np_img
...
def save_display_tile(tile, save=True, display=False):
  ...
  if save:
    ...
    if not os.path.exists(dir):
      os.makedirs(dir)
    tile_pil_img.write_to_file(img_path)
